...
/// shortened. Keeps compact image info from overflowing on mobile.
...
enum InfoStyle {
    Wide,
    Narrow { max_name: usize },
}

fn image_info_label(side: Side, img: &LoadedImage, style: InfoStyle) -> String {
    match style {
        InfoStyle::Wide => format!(
            "{}: {} ({}) ({}x{})",
            side.label(),
            img.name,
        ...
        InfoStyle::Narrow { max_name } => format!(
            "{}: {} · {} · {}×{}",
            side.short_label(),
            truncate_name(&img.name, max_name),
            format_file_size(img.file_size),
            img.size[0],
            img.size[1]
        ),
...
impl App {
    fn fit_image_rect(&self, canvas: Rect, img_size: [usize; 2]) -> Rect {
        let img_w = img_size[0] as f32;
        let img_h = img_size[1] as f32;
        let scale = (canvas.width() / img_w).min(canvas.height() / img_h) * self.view.zoom;
        let display = Vec2::new(img_w * scale, img_h * scale);
        let center = canvas.center() + self.view.pan_offset;
        Rect::from_center_size(center, display)
    }

    pub(crate) fn draw_comparison(&mut self, ui: &mut egui::Ui) {
        ...
        self.handle_canvas_drag(ui, &response, canvas, pointer_pos);
        self.handle_zoom(ui, &response, canvas, pointer_pos);
    }

    /// Draw both images, each clipped to its half of the separator.
    fn draw_images(&self, painter: &egui::Painter, canvas: Rect, separator_x: f32) {
        let left = self.image(Side::Left).expect("both images loaded");
        let right = self.image(Side::Right).expect("both images loaded");

        let tex_id = |side: Side, base: &egui::TextureHandle| {
        ...
        painter.with_clip_rect(right_clip).image(
            tex_id(Side::Right, &right.texture),
            right_rect,
            uv,
            Color32::WHITE,
        );
    ...
    /// Separator drag and image panning.
    ...
    /// Scroll-wheel and pinch zoom, keeping the point under the cursor fixed.
    fn handle_zoom(
        &mut self,
        ui: &mut egui::Ui,
        response: &egui::Response,
    ...
    /// Shared Open / Edge / Reset / Clear controls used by both headers.
    fn header_actions(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, edge_label: &str) {
        ...
        let mut enabled = self.edges.enabled;
        ...
        if self.any_loaded() && ui.button("Clear").clicked() {
            self.clear_all();
        }
    }

    fn draw_side_info(&self, ui: &mut egui::Ui, style: InfoStyle) {
        for side in Side::ALL {
            if let Some(img) = self.image(side) {
                ui.label(image_info_label(side, img, style));
            }
        }
    }

    /// Wide-screen header: single horizontal row.
    pub(crate) fn draw_header_wide(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.horizontal(|ui| {
            ...
            for side in Side::ALL {
                if let Some(img) = self.image(side) {
                    ...
                    first = false;
                    ui.label(image_info_label(side, img, InfoStyle::Wide));
                }
            }

            // Zoom on the far right; action buttons in natural LTR order just left of it.
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.label(format!("Zoom: {:.0}%", self.view.zoom * 100.0));
                ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                    self.header_actions(ui, ctx, "Edge Detect");
                });
            });
        });
    }

    /// Narrow/mobile header: stacked title, wrapping controls, truncated info.
    pub(crate) fn draw_header_narrow(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        ui.horizontal_wrapped(|ui| {
            self.header_actions(ui, ctx, "Edges");
        });

        self.draw_side_info(ui, InfoStyle::Narrow { max_name: 18 });
        ui.add_space(2.0);
    }

    pub(crate) fn draw_loading(&mut self, ui: &mut egui::Ui) {
        let available = ui.available_size();
    ...
    pub(crate) fn draw_drop_zone(&mut self, ui: &mut egui::Ui) {
        let available = ui.available_size();
        let (response, painter) = ui.allocate_painter(available, Sense::hover());
        let rect = response.rect;
        ...
        let text = match self.auto_route_side() {
            Side::Left => "First image",
            Side::Right => "Second image",
        };
        ...
        if response.interact(Sense::click()).clicked() {
            self.request_open(ui.ctx(), self.auto_route_side());
        }
    }
}
